...
func inflect(r message.Role, f string, v ...interface{}) string {
	...
	if r == message.CommandRole {
		...
		f = strings.Replace(f, "<other-message>", "event", -1)
		f = strings.Replace(f, "<other-messages>", "events", -1)
		f = strings.Replace(f, "<other-produce>", "record", -1)
		f = strings.Replace(f, "<other-produced>", "recorded", -1)
	} else {
		...
		f = strings.Replace(f, "<other-message>", "command", -1)
		f = strings.Replace(f, "<other-messages>", "commands", -1)
		f = strings.Replace(f, "<other-produce>", "execute", -1)
		f = strings.Replace(f, "<other-produced>", "executed", -1)
	}
